                ...
                for (unsigned p = 0 ; p < pattern.size() ; ++p) {
                    ...
                    for (unsigned t = 0 ; t < target.size() ; ++t) {
                        ...
                        for (auto & c : adjacency_constraints) {
                            ...
                            // check labels
                            if (pattern.label(p) != target.label(t))
                                ok = false;
                            ...
                            if (ok) {
                                ...
                                // check degree
                                if (ok && params.degree && ! (c.first[p].count() <= c_second_t.count()))
                                    ok = false;
                            }
