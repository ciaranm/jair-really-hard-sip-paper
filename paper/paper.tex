...
effective, even on families of graphs for which it was not designed. However, these comparisons have
either ignored strong propagation algorithms, or in one case misunderstood them to be part of a
filtering step rather than matching algorithms \citep{DBLP:conf/icdm/AlmasriGF14}.

We believe it would be unlikely to cause too much astonishment if we suggested that a better
matching algorithm could be dropped in as a black box replacement in graph databases systems to
improve their performance.  However, it appears not to have occurred to the graph databases
community that a better subgraph isomorphism algorithm could invalidate the premise underlying the
entire filter / verify approach.  Eliminating this filtering step would not merely be a matter of
...
?? Presenting the ultimate big data subgraph matching algorithm is outwith the scope of this paper.

?? They need to read outside the field. AI has a lot to offer. There needs to be much more focus on
the matching algorithm, and how it interacts with the system as a whole.

?? We need optimisations for trivial cases. Lazy forward checking, cheap alternatives to heuristics and
all-different. Presolving. Huge domains. Ongoing use of VF2 and micro vs millisecond benchmarks.
