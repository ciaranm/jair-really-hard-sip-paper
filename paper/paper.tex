...
\begin{abstract}
    ...
    subgraph isomorphism algorithm. With this in mind, we argue that widely researched ``filter /
    verify'' indexing technique used in graph databases is a flawed design based upon a widely
    repeated misunderstanding of what makes subgraph isomorphism hard. We explain why this technique
    would not be beneficial, even with harder instances than those currently used, when paired with
...
an edge between each distinct pair of vertices with independent probability $p$.

A graph may have labels on its vertices, in which case we write $\ell(v)$ for the label of a given
vertex $v$. We write $G(n, p, l)$ for the graph $G(n, p)$ with vertex labels selected uniformly at
random from a set of $l$ different labels.
...
Two adjacent differently-coloured vertices.

VF2 does not detect the case that there are insufficient, or even no, labels.
...
pattern but only one in the target.
...
propagator, but such techniques are not used in graph databases systems, let alone the more powerful
degree- and difference-based propagation rules introduced LAD \cite{Solnon:2010} or the
distance-based reasoning in more recent work \cite{Audemard:2014,McCreesh:2015}.

Indeed, as \citet{DBLP:journals/pvldb/KatsarouNT15} note, some variation on VF2
\citep{Cordella:2004} is the usual matching algorithm of choice, although
\citeS{DBLP:journals/jacm/Ullmann76}'s algorithm is sometimes chosen. Other approaches have been
considered: for example, \citet{DBLP:journals/pvldb/ShangZLY08} propose an algorithm which makes use
of the frequency of various features to guide search.  \citet{DBLP:journals/pvldb/LeeHKL12}
...
We do not present the ultimate big data subgraph matching algorithm.

Read outside the field. AI has a lot to offer.

Much more focus on the matching algorithm.

Optimisations for trivial cases. Lazy forward checking, cheap alternatives to heuristics and
all-different. Presolving.

Invariants on labels used to inform algorithm design and to provide filtering during search.

Indexing can be used to enhance propagation.
