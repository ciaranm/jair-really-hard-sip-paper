...
in \cref{figure:sip}.  Although these problems are \NP-complete \citep{DBLP:books/fm/GareyJ79}, modern subgraph
...
value-ordering heuristics---this is the second contribution of this paper.
...
scheme, and the popular VF2 algorithm \citep{DBLP:journals/pami/CordellaFSV04} behaves much worse
than two more recent constraint-based algorithms in certain cases. In \cref{section:labelled} we
take a close look at these cases, and argue that they \emph{should} be easy to solve.
...
also appeared in a PhD thesis \citep{o:McCreesh17}.
...
\emph{value-ordering} heuristics, which select which value to try giving that variable. That is,
variable-ordering selects a pattern vertex, whilst value-ordering tells us the order in which to try
different candidate target vertices for that pattern vertex. If at any point during search a
\emph{domain wipeout} occurs (that is, if every value is removed from any domain), the search will
backtrack immediately.

In the case of subgraph isomorphism, the basic constraint model is to have a domain for each pattern
vertex, with each domain being the set of target vertices. An \emph{all-different} constraint ensures
...
problems \citep{DBLP:conf/aaai/GentMPW96}. An ensemble of problem instances with $\kappa < 1$ is
...
problem instances from an ensemble with $\kappa$ close to 1 are hard, and problems drawn from an
ensemble where $\kappa$ is very small or very large are usually easy.
...
widespread use of weak subgraph isomorphism algorithms has not just led to overly pessimistic
...
separate I/O and isomorphism testing costs into $T_{\mathit{io}}$ and $T_{\mathit{iso\_test}}$
...
reasonably good subgraph isomorphism algorithm. Thus, all filtering \emph{should} be doing is eliminating the startup
...
following the example of \citet{o:10.1371/journal.pone.0076911}, they are widely used. Each dataset
...
behaviour seen in these instances is purely down to the choice of subgraph isomorphism algorithm,
...
\citet{DBLP:conf/edbt/KatsarouNT17} present approaches to improve the run times for what they call
...
first'' \citep{DBLP:journals/ai/HaralickE80}.  Of the orderings proposed, those involving placing
...
domain-based ordering heuristics, rather than assembling a portfolio of weak algorithms in the hopes
...
best existing algorithms for subgraph isomorphism, and with more broadly-applicable lessons on
algorithm design from the constraint programming literature.

Another such lesson is the importance of choosing a good set of benchmark instances. As
\citet{DBLP:journals/heuristics/Gent98} observes:
...
evaluations work only with extremely easy instances. For example, \citet{DBLP:journals/tcbb/BonniciG17}
...
easy. We looked at this kind of instance in more detail, and argued that this was due to shortcomings in
VF2's design, rather than an interesting property of \NP-completeness. Although inevitably there
will be instances that every algorithm finds hard, we see it as a serious weakness for an algorithm to exhibit
exponential behaviour in such trivial cases as when there are two red vertices in the pattern but only one in the
target.
...
conclusion that extensive research into supporting techniques is important, whereas really what
they are doing is working around some but not all of the weaknesses in their chosen algorithms for subgraph
isomorphism. With this new understanding of what does and does not make subgraph
...
in fact good in this role \citep{DBLP:conf/lion/KotthoffMS16}---but there are other possibilities.
...
\acks{The authors would like to thank Kitty Meeks and Craig Reilly for their comments. We are
grateful to Iva Babukova for her help with the datasets and for sharing the results of her
experiments. The heatmap schemes used in this paper were created with the assistance of the
``Chroma.js Color Scale Helper'' by Gregor Aisch.}
