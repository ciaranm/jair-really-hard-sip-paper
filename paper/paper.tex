...
\begin{abstract}
    ...
    verify'' indexing technique used in graph databases is founded upon a
...
instances, unless the chosen subgraph isomorphism algorithm exhibits unnecessarily poor behavior
on certain classes of instance.
...
widespread use of poor subgraph isomorphism algorithms has not just led to overly pessimistic
...
that better algorithms invalidate the premise underlying the entire filter~/ verify approach.
...
instances.  We thus believe that a significant amount of effort in designing algorithms for subgraph
isomorphism has been misdirected due to a lack of familiarity with the best existing algorithms for
subgraph isomorphism on one hand, and with broadly-applicable lessons from the constraint
programming literature on the other.

We now move our focus from the importance of using state-of-the-art techniques for algorithm design,
to the pitfalls of choosing unrealistically easy instances to benchmark our algorithms.
...
This lesson has not been well heeded. For example, \citet{DBLP:journals/tcbb/BonniciG17}
...
they are doing is working around some but not all of the defects in their chosen algorithms for subgraph
