...
purely constraint programming style approach:
...
Improved search orderings for VF2 also appear in recent work by
\citet{DBLP:conf/gbrpr/CarlettiFV15} and \citet{o:Carletti16}, cumulating in VF3
\citep{DBLP:conf/gbrpr/CarlettiFSV17}, and by \citet{DBLP:conf/wise/ShenZ17}. We have seen in
\cref{section:labelled} that such an approach could give improvements, but will not bring an
algorithm that does not use domains close to the performance of one that does on harder instances.
Indeed, we saw in \cref{figure:induced} that VF3 performs \emph{worse} than VF2 on harder instances.
